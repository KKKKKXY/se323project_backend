...
public class StudentAnotherController {
    ...
    public ResponseEntity<?> getStudentByName(@PathVariable String name){
        return ResponseEntity.ok(MapperUtil.INSTANCE.getStudentDto(studentAnotherService.getStudentByNameContains(name)));
    }

    @GetMapping("/student/advisor/{name}")
    public ResponseEntity<?> getStudentByAdvisorName(@PathVariable String name){
        return ResponseEntity.ok(MapperUtil.INSTANCE.getStudentDto(studentAnotherService.getStudentWhoseAdvisorNameIs(name)));
    }
}
